...
public class VidapetService {
        ...
        private final citaDAO citaDAO; // اگر نام کلاس را به CitaDAO تغییر داده‌اید، اینجا اصلاح شود
    ...
    public Map<String, Object> obtenerPropietarioPorMascota(Long id) {
        ...
        // تغییر: دریافت Long especieId به جای String especie
        public void guardarMascota(String nombre, Long especieId, String raza,
        ...
        // تغییر: دریافت Long especieId به جای String especie
        public void actualizarMascota(Long id, String nombre, Long especieId, String raza,
                                      LocalDate fechaNacimiento, Long propietarioId, String foto) {
    ...
    public Map<String, Object> obtenerConsultaDetalleCompleto(int id) {
        ...
        public void actualizarTratamiento(int id, int consultaId, String tratamiento, LocalDate fechaInicio, LocalDate fechaFin, String observaciones) {
        ...
        public List<Map<String, Object>> listarEspecies(String search) {
            if (search != null && !search.isBlank()) {
                return especieDAO.search(search);
            }
            return especieDAO.findAll();
        }

        public Map<String, Object> obtenerEspeciePorId(Long id) {
        ...
        public void actualizarEspecie(Long id, String nombre, String foto) {
            especieDAO.update(id, nombre, foto);
        }
    ...
    public Map<String, Object> obtenerDetalleParaNuevaConsulta(int mascotaId, int citaId) {
        // این متد باید اطلاعات پت، صاحب، گونه و دامپزشک را یکجا ترکیب کند
        Map<String, Object> details = new HashMap<>();

        // فرض می‌کنیم در DAO متدی داری که اطلاعات کامل را با JOIN می‌آورد
        // یا می‌توانید چند متد را اینجا با هم ترکیب کنید
    ...
    public List<Map<String, Object>> buscarConsultasSeguro(String nombre) {
        // ۱. منطق بیزینس: مثلاً اگر ورودی خالی بود، جستجو نکن
        if (nombre == null || nombre.trim().isEmpty()) {
            return consultaDAO.findAll();
        }

        // ۲. منطق امنیتی یا فیلترینگ: مثلاً نام‌های خیلی کوتاه را نگرد
        if (nombre.length() < 2) {
            return new ArrayList<>();
        }

        // ۳. هماهنگی: صدا زدن DAO برای گرفتن داده‌ها
